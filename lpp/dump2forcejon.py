...
# assuming a dump.fc.*.liggghts call, this loops over the 'n' number of files that * is taking the place of
...
<UnstructuredGrid>
        <Piece NumberOfCells="0" NumberOfPoints="0">
            ...
            contactArea = np.array(forcedata.snaps[fileindex].atoms[:,forcedata.names["contactArea"]],dtype=np.float64)
            averageContactArea = np.average(contactArea)
            averageContactRadius = np.sqrt(averageContactArea/np.pi)
            a[timeSnap] = averageContactRadius
            # print('Average contact radius: ' + str(averageContactRadius))
            ...
            w.addData("contactArea", contactArea)
            ...
            w.appendData(force).appendData(contactArea).appendData(heatflux)
            w.save()
